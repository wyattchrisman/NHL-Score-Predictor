...
pacman::p_load(tidyverse, dplyr, ggfittext, patchwork, FNN)
...
all_years_data <- 
  read.csv("https://moneypuck.com/moneypuck/playerData/careers/gameByGame/all_teams.csv")      #read.csv(url("https://wchrisma.w3.uvm.edu/DS-data/all_teams.csv")) 
...
Our dataframe has many differt columns. Here are some of the most important ones
1. Team - The team that is playing in that game
2. Season - The season year that the game is being played in
3. opposingTeam - The opponent that team is playing against in the game
3. totalGoals - The total number of goals that are scored during the game
4. OU - The result of the game if the over or under hit
5. totalShots - The total number of shots taken during the game
...
ggplot(
  ...
  labs(title = "Team Average Total Goals per Game during 2023 Season",
       x = "Team",
...
# Want to add teams logo to top of each bar
```
...
Every shot taken during the game will fall into one of three categories depending on the play situation as well as the area on the ice that the shot is taken. The same aspect applies to all goals scored. Here we are showing the regression lines of the correlation between each of these shot types and their goal types. You can see that each regression line follows a pattern. With the low danger shots has a linear slope that is not very steep. This shows that when low danger goals are not as frequent, which is not a surprise. Then, medium danger shots has a steeper linear slope that stays continuously linear. Finally, we have high danger shots which has an even steeper slope that the medium danger but plateaus out at the end. So we can see that the relationship between each shot type and their corresponding goals is linear. This allows us to conclude that the correlation between each shot type and their goal type is high. This could cause multicollinearity issues when we begin out regression model for predicting total goals.
