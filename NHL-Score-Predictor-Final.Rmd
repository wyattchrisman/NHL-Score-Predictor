...
Arjun TODO - Separate team name labels, 
...
average_goals_data$team <- toupper(average_goals_data$team)
# Group by team and season, then summarize to calculate the mean goals per game
average_goals_data <- data %>%
  filter(season == 2023) %>%
  group_by(team) %>%
  summarize(average_goals = mean(totalGoals), .groups = 'drop')

#Using ggplot to create a bar chart
ggplot(average_goals_data, aes(x = team, y = average_goals)) +
  geom_bar(stat = "identity", aes(fill = team)) +
  scale_fill_manual(values = team_colors) +
  geom_hline(yintercept = 5.5, linetype = "dashed", color = "black") +
  labs(title = "Average Goals per Game by Team for the 2023",
       x = "Team",
