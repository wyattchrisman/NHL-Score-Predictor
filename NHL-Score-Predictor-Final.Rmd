...
data <- 
  data |>
  dplyr::select(team, season, opposingTeam:gameDate, xGoalsFor, shotsOnGoalFor, shotAttemptsFor, goalsFor, takeawaysFor, giveawaysFor, lowDangerShotsFor:highDangerGoalsFor, xGoalsAgainst, shotsOnGoalAgainst, shotAttemptsAgainst, goalsAgainst, takeawaysAgainst, giveawaysAgainst, lowDangerGoalsAgainst:highDangerGoalsAgainst) |>
...
Wyatt TODO: Add labels to top of bars
...
ggplot(
  ...
  labs(
    x = "NHL Teams",
    title = "NHL Team Over/Under Probabilities (%)"
  ) + 
...
```
The purpose of this scatter plot is to attempt to visualize the correlation between total shots in a game and whether the over is cleared. From this scatterplot it is hard to see a clear correlation outside of the major outliers. This may indicate the presence of other factors that are more impactful towards the over/under outcome.

## Graph3 - Same as above with low, medium, high danger shots - could use facet wrap
```{r L/M/H Danger shots}

data |>
  select(team:OU, shotsOnGoalFor, lowDangerShotsFor:highDangerGoalsFor)
...
## Graph4 - Average goals/game of each team per season
